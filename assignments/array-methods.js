...
let allCaps = runners.map(users => users.first_name.toUpperCase());

console.log(allCaps);

// ==== Challenge 3: Use .filter() ====
// The large shirts won't be available for the event due to an ordering issue.  Get a list of runners with large sized shirts so they can choose a different size. Return an array named largeShirts that contains information about the runners that have a shirt size of L and log the result
let largeShirts = runners.filter(users => users.shirt_size === "L");
console.log(largeShirts);

// ==== Challenge 4: Use .reduce() ====
// The donations need to be tallied up and reported for tax purposes. Add up all the donations into a ticketPriceTotal array and log the result
let ticketPriceTotal = runners.reduce((counter, users) => {
  return counter + users.donation;
}, 0);
...
// Problem 1
// I want to send a mass email to everyone in the 5k, I need all the emails in a new array to be able to send them the email for the location.
let emails = [];
let emailPool = runners.forEach(users => emails.push(users.email));

console.log(emails);
// Problem 2
// We have a set goal we want to hit in donations for the 5k we want to see how much more money we need to raise till we hit $10,000.
let counterToDonationGoal = runners.reduce((goal, users) => {
  return goal - users.donation;
}, 10000);

console.log(`We need $${counterToDonationGoal} till we hit our goal!`);
// Problem 3
console.log("Problem 3");
//We have decided we don't want anyone with the letter A in their first name in the 5K don't ask why we just dont make a new array with all their data people without that cursed letter.
let noAs = runners.filter(users => {
  return users.first_name.indexOf("a") === -1;
});
